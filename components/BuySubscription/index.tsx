...
export const BuySubscriptionComponent: React.FC<BuySubscriptionProps> =
  observer(({ sub }) => {
    ...
    const send = () => {
      axiosJWT
        .post(`${URL}/auth/buy-sub`, null, { withCredentials: true })
        .then(async () => {
          await MySwal.fire({
    ...
    return (
      <>
        ...
        <div className="container">
          ...
          <button className={`btn ${theme.theme}`} onClick={() => send()}>
            Оформить
          </button>
        </div>
      </>
